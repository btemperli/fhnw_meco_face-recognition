 ...
 */
function initFaceTrackerImage(images) {
	...
	var _stage = nxtjs.initCreateJS("imageTracker");
	_stage.addChild(new nxtjs.FaceTrackerImage(imageArray));
}
...
(function(lib, cjs) {
	...
	(lib.FaceTrackerImage = function(
        ...
        faceDetectionRoi,
        screenRect,
        maskContainer,webcamInput
        ) {

		var _this = this;
		var _super = lib.FaceTrackerImage._super;
		
		_this._imageURLs = images;
		_this._imageBMDs = [];
		_this._images = [];
		_this._image = null;
		...
		_this.updateGUI = function() {
            ...
            if (database === null) {
                ...
                database = jQuery.parseJSON(database);
            }

            var i = _this._images.indexOf(_this._image);
            var name = imageNames[i];

            database.push(name);

            localStorage.setItem('database', JSON.stringify(database));
            localStorage.setItem(name, JSON.stringify(faceShape));
